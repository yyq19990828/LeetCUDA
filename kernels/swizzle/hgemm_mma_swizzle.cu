...
// only 1 warp per block(32 threads), m16n8k16. A, B, C: all row_major.
template <const int MMA_M = 16, const int MMA_N = 8, const int MMA_K = 16>
__global__ void hgemm_mma_m16n8k16_naive_smem_swizzle_kernel(half *A, half *B,
                                                             half *C, int M,
                                                             int N, int K) {
  const int bx = blockIdx.x;
  const int by = blockIdx.y;
...
#pragma unroll
  for (int k = 0; k < NUM_K_TILES; ++k) {
    ...
    // LDST128BITS(s_a[load_smem_a_m][load_smem_a_k]) = (
    //   LDST128BITS(A[load_gmem_a_addr]));
    LDST128BITS(s_a[load_smem_a_m][swizzle_A_j(load_smem_a_m, load_smem_a_k)]) =
        (LDST128BITS(A[load_gmem_a_addr]));
...
template <const int MMA_M = 16, const int MMA_N = 8, const int MMA_K = 16,
          ...
          const int A_PAD = 0, const int B_PAD = 0>
__global__ void __launch_bounds__(256)
    hgemm_mma_m16n8k16_mma2x4_warp4x4_smem_swizzle_kernel(half *A, half *B,
                                                          half *C, int M, int N,
                                                          int K) {
  const int bx = blockIdx.x;
  const int by = blockIdx.y;
...
// launcher
void launch_hgemm_mma_m16n8k16_naive(half *a, half *b, half *c, int M, int N,
                                     int K) {
...
void launch_hgemm_mma_m16n8k16_naive_smem_swizzle(half *a, half *b, half *c,
                                                  int M, int N, int K) {
  constexpr int MMA_M = 16;
  constexpr int MMA_N = 8;
  constexpr int MMA_K = 16;
  ...
  hgemm_mma_m16n8k16_naive_smem_swizzle_kernel<MMA_M, MMA_N, MMA_K>
      <<<grid, block>>>(a, b, c, M, N, K);
}

void launch_hgemm_mma_m16n8k16_mma2x4_warp4x4(half *a, half *b, half *c, int M,
                                              int N, int K) {
  constexpr int MMA_M = 16;
  constexpr int MMA_N = 8;
  constexpr int MMA_K = 16;
  ...
  // ./hgemm_mma_swizzle.bin ncu --metrics
  // sm__sass_l1tex_data_bank_conflicts_pipe_lsu_mem_shared_op_ldsm
  // ./hgemm_mma_swizzle.bin constexpr int A_PAD = 8; constexpr int B_PAD = 8;
...
void launch_hgemm_mma_m16n8k16_mma2x4_warp4x4_smem_swizzle(half *a, half *b,
                                                           half *c, int M,
                                                           int N, int K) {
  constexpr int MMA_M = 16;
  constexpr int MMA_N = 8;
...
int main(int argc, char *argv[]) {
  ...
  avg_sec = perf_gemm<half>(launch_hgemm_mma_m16n8k16_naive, M, N, K, W, R);
  ...
  avg_sec = perf_gemm<half>(launch_hgemm_mma_m16n8k16_naive_smem_swizzle, M, N,
                            K, W, R);
  avg_Tflops = ((double)M) * N * K * 2 * 1e-12 / avg_sec;
  printf("M N K = %6d %6d %6d, W = %d, R = %d, ", M, N, K, W, R);
  printf("Time = %12.8lf s, AVG Performance = %10.4lf Tflops\n", avg_sec,
  ...
  avg_sec =
      perf_gemm<half>(launch_hgemm_mma_m16n8k16_mma2x4_warp4x4, M, N, K, W, R);
  avg_Tflops = ((double)M) * N * K * 2 * 1e-12 / avg_sec;
  printf("M N K = %6d %6d %6d, W = %d, R = %d, ", M, N, K, W, R);
  printf("Time = %12.8lf s, AVG Performance = %10.4lf Tflops\n", avg_sec,
         avg_Tflops);
  ...
  avg_sec = perf_gemm<half>(
      launch_hgemm_mma_m16n8k16_mma2x4_warp4x4_smem_swizzle<0>, M, N, K, W, R);
  avg_Tflops = ((double)M) * N * K * 2 * 1e-12 / avg_sec;
  printf("M N K = %6d %6d %6d, W = %d, R = %d, ", M, N, K, W, R);
  printf("Time = %12.8lf s, AVG Performance = %10.4lf Tflops\n", avg_sec,
         avg_Tflops);
  ...
  avg_sec = perf_gemm<half>(
      launch_hgemm_mma_m16n8k16_mma2x4_warp4x4_smem_swizzle<8>, M, N, K, W, R);
  avg_Tflops = ((double)M) * N * K * 2 * 1e-12 / avg_sec;
  printf("M N K = %6d %6d %6d, W = %d, R = %d, ", M, N, K, W, R);
  printf("Time = %12.8lf s, AVG Performance = %10.4lf Tflops\n", avg_sec,
         avg_Tflops);
