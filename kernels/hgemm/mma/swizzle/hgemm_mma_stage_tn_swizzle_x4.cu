...
template <const int MMA_M, const int MMA_N, const int MMA_K,
          const int MMA_TILE_M, const int MMA_TILE_N, const int WARP_TILE_M,
          const int WARP_TILE_N, const int WARP_TILE_K, const int A_PAD,
          const int B_PAD, const int K_STAGE, const bool BLOCK_SWIZZLE,
          const bool WARP_SWIZZLE>
__global__ void __launch_bounds__(256)
  ...
  static_assert(MMA_TILE_M == 2 && MMA_TILE_N == 4);
  static_assert(WARP_TILE_M == 4 && WARP_TILE_M == 4 && WARP_TILE_K == 2);
  static_assert(A_PAD == 0 && B_PAD >= 0 && B_PAD % 8 == 0);
  static_assert(K_STAGE >= 2);
      ...
      for (int j = 0; j < WARP_TILE_N; ++j) {
        ...
        if constexpr (B_PAD == 0) {
          uint32_t lane_smem_b_ptr =
              (smem_b_base_ptr +
               s_b_mma_k_store_offset * sizeof(half) * (lane_id / 16) +
               (stage_sel_reg * s_b_stage_offset +
...
template <const int K_STAGE, const int BLOCK_SWIZZLE_STRIDE, const int B_PAD>
void lanunch_hgemm_mma_m16n8k16_swizzle_tn_x4(half *a, half *b, half *c, int M,
                                              int N, int K) {
  ...
  constexpr bool WARP_SWIZZLE = true;

  const int smem_max_size =
      ((K_STAGE)*BM * (BK + A_PAD) * WARP_TILE_K * sizeof(half) +
       (K_STAGE)*BN * (BK + B_PAD) * WARP_TILE_K * sizeof(half));

  cudaFuncSetAttribute(
      hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4_kernel<
          MMA_M, MMA_N, MMA_K, MMA_TILE_M, MMA_TILE_N, WARP_TILE_M, WARP_TILE_N,
          WARP_TILE_K, A_PAD, B_PAD, K_STAGE, BLOCK_SWIZZLE, WARP_SWIZZLE>,
...
// ./hgemm_mma_stage_tn_swizzle.80.debug.bin 1024 1024 1024 0 1
  ...
  int B_PAD = 0; // 0, use smem swizzle for B; 8, use smem padding for B.
#ifdef HGEMM_MMA_DEBUG
  // control all M, N, K, warmup, repeat, B_PAD in debug mode.
  ...
  if (argc > 6)
    B_PAD = (std::stoi(argv[6]) == 8 ? 8 : 0);
#else
  // only control all B_PAD in debug mode.
  int outer_repeat = 10, inner_repeat = 1, warmup = 1;
  if (argc > 1)
    B_PAD = (std::stoi(argv[1]) == 8 ? 8 : 0);
#endif

  if (B_PAD == 0) {
    printf("ALGO = MMA16816 HGEMM TN MMA=2x4 WARP=4x4x2 STAGES=2, ");
    printf("BLOCK SWIZZLE=2048, A&B SMEM SWIZZLE, B LDMATRIX.X4\n");
  } else {
    printf("ALGO = MMA16816 HGEMM TN MMA=2x4 WARP=4x4x2 STAGES=2, ");
    printf("BLOCK SWIZZLE=2048, A SMEM SWIZZLE, B Pad = 8, B LDMATRIX.X4\n");
  }
  ...
  for (int j = 0; j < 5; j++) {
    ...
    if (B_PAD == 0) {
      max_error = gemm_error_check_tn<half>(
          lanunch_hgemm_mma_m16n8k16_swizzle_tn_x4<2, 2048, 0>, M, N, K);
    } else {
      max_error = gemm_error_check_tn<half>(
          lanunch_hgemm_mma_m16n8k16_swizzle_tn_x4<2, 2048, 8>, M, N, K);
    }
  ...
  for (int j = 0; j < test_num; j++) {
    ...
    for (int k = 0; k < outer_repeat; k++) {
      double this_sec;
      if (B_PAD == 0) {
        this_sec = perf_gemm<half>(
            lanunch_hgemm_mma_m16n8k16_swizzle_tn_x4<2, 2048,
                                                     0>, // A&B fully swizzle.
            M, N, K, inner_repeat, warmup);
      } else {
        this_sec = perf_gemm<half>(
            lanunch_hgemm_mma_m16n8k16_swizzle_tn_x4<2, 2048, 8>, // A swizzle,
                                                                  // B padding.
            M, N, K, inner_repeat, warmup);
      }
...
#define LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X4_KERNEL(stages, \
                                                                       stride) \
  {                                                                            \
    const int smem_max_size =                                                  \
    ...
    cudaFuncSetAttribute(                                                      \
        hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4_kernel< \
            ...
            WARP_TILE_N, WARP_TILE_K, A_PAD, B_PAD, (stages), true, true>,     \
        cudaFuncAttributeMaxDynamicSharedMemorySize, 98304);                   \
    ...
    hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4_kernel<     \
        MMA_M, MMA_N, MMA_K, MMA_TILE_M, MMA_TILE_N, WARP_TILE_M, WARP_TILE_N, \
        WARP_TILE_K, A_PAD, B_PAD, (stages), true, true>                       \
...
#define LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X4_KERNEL(     \
    stages)                                                                    \
  {                                                                            \
    const int smem_max_size =                                                  \
        ((stages) * BM * (BK + A_PAD) * WARP_TILE_K * sizeof(half) +           \
    ...
    cudaFuncSetAttribute(                                                      \
        hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4_kernel< \
            MMA_M, MMA_N, MMA_K, MMA_TILE_M, MMA_TILE_N, WARP_TILE_M,          \
            WARP_TILE_N, WARP_TILE_K, A_PAD, B_PAD, (stages), false, true>,    \
        cudaFuncAttributeMaxDynamicSharedMemorySize, 98304);                   \
    dim3 block(NUM_THREADS);                                                   \
    dim3 grid(div_ceil(N, BN), div_ceil(M, BM));                               \
    hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4_kernel<     \
        MMA_M, MMA_N, MMA_K, MMA_TILE_M, MMA_TILE_N, WARP_TILE_M, WARP_TILE_N, \
        WARP_TILE_K, A_PAD, B_PAD, (stages), false, true>                      \
        <<<grid, block, smem_max_size>>>(                                      \
            reinterpret_cast<half *>(a.data_ptr()),                            \
            reinterpret_cast<half *>(b.data_ptr()),                            \
            reinterpret_cast<half *>(c.data_ptr()), M, N, K);                  \
...
void hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4(
  ...
  constexpr int B_PAD =
      8; // B swizzle seems can not get good performance than padding, why?
  constexpr int NUM_THREADS =
      (MMA_TILE_M * MMA_TILE_N * WARP_SIZE); // 2 * 4 * 32 = 256
  constexpr int BM = MMA_M * MMA_TILE_M * WARP_TILE_M;
  constexpr int BN = MMA_N * MMA_TILE_N * WARP_TILE_N;
