...
#define LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X2_KERNEL(stages, \
                                                                       stride) \
  {                                                                            \
...
void lanunch_hgemm_mma_m16n8k16_swizzle_tn_x2(half *a, half *b, half *c, int M,
                                              int N, int K) {
  ...
  LAUNCH_16816_STAGE_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X2_KERNEL(
...
// ./hgemm_mma_stage_tn_swizzle.80.debug.bin 1024 1024 1024 0 1
  ...
  printf("ALGO = MMA16816 HGEMM TN MMA=2x4 WARP=4x4x2 STAGES=2, BLOCK "
         "SWIZZLE=2048, A SMEM SWIZZLE, B Pad = 8\n");
#ifndef HGEMM_MMA_DEBUG
  for (int j = 0; j < 5; j++) {
    int M = M_list[j], N = N_list[j], K = K_list[j];
    float max_error = gemm_error_check_tn<half>(
        lanunch_hgemm_mma_m16n8k16_swizzle_tn_x2<2, 2048>, M, N, K);
  ...
  for (int j = 0; j < test_num; j++) {
    ...
    for (int k = 0; k < outer_repeat; k++) {
      double this_sec =
          perf_gemm<half>(lanunch_hgemm_mma_m16n8k16_swizzle_tn_x2<2, 2048>, M,
                          N, K, inner_repeat, warmup);
...
#define LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X2_KERNEL(     \
    stages)                                                                    \
  {                                                                            \
    const int smem_max_size =                                                  \
        ((stages) * BM * (BK + A_PAD) * WARP_TILE_K * sizeof(half) +           \
...
void hgemm_mma_m16n8k16_mma2x4_warp4x4x2_stages_dsmem_tn_swizzle_x4(
  ...
  if (swizzle) {
    ...
    switch (stages) {
    case 2:
      LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X2_KERNEL(2);
      break;
    case 3:
      LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X2_KERNEL(3);
      break;
    case 4:
      LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X2_KERNEL(4);
      break;
    case 5:
      LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X2_KERNEL(5);
      break;
    default:
      LAUNCH_16816_STAGE_NO_SWIZZLE_MMA2x4_WARP4x4x2_DSMEM_TN_X2_KERNEL(2);
      break;
