...
__global__ void __launch_bounds__(WARP_SIZE *kMmaTileSeqLenQ *kMmaTileSeqLenK)
    flash_attn_mma_stages_split_q_shared_kv_acc_f32_kernel(half *Q, half *K,
                                                           half *V, half *O,
                                                           int QKV_seqlen,
                                                           int QKV_head) {
...
void launch_flash_attn_mma_stages_split_q_shared_kv_acc_f32(torch::Tensor Q,
                                                            torch::Tensor K,
                                                            torch::Tensor V,
                                                            torch::Tensor O) {
  ...
  cudaFuncSetAttribute(
      flash_attn_mma_stages_split_q_shared_kv_acc_f32_kernel<
...
void flash_attn_mma_stages_split_q_shared_kv_acc_f32(torch::Tensor Q,
  ...
  if (stages > 1) {
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_shared_kv_acc_f32<32, 2>(Q, K, V, O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_shared_kv_acc_f32<64, 2>(Q, K, V, O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_shared_kv_acc_f32<96, 2>(Q, K, V, O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_shared_kv_acc_f32<128, 2>(Q, K, V,
                                                                     O);
      break;
    default:
    ...
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_shared_kv_acc_f32<32, 1>(Q, K, V, O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_shared_kv_acc_f32<64, 1>(Q, K, V, O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_shared_kv_acc_f32<96, 1>(Q, K, V, O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_shared_kv_acc_f32<128, 1>(Q, K, V,
                                                                     O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_shared_kv_acc_f32<256, 1>(Q, K, V,
                                                                     O);
      break;
    default:
      throw std::runtime_error("headdim not support!");
