...
// Fine-grained tiling at the MMA level for Q and K results in a constant SRAM
// usage of 64 * kMmaAtomK for Q and K. For V, the SRAM complexity is
// O(kMmaAtomK * d), leading to an overall SRAM complexity of O(kMmaAtomK * d).
// Consequently, this approach allows us to extend D (head dimension) up to
// 1024. Performance optimizations are ongoing. Stay tuned for updates ~
...
// i: row index; j: col index
// e.g kColStride = kMmaAtomK = 16, kStep = 8 -> load 8 half as 128 bits memory
// issue.
...
__global__ void __launch_bounds__(WARP_SIZE *kMmaTileSeqLenQ *kMmaTileSeqLenK)
    flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk_kernel(half *Q, half *K,
                                                              half *V, half *O,
                                                              int QKV_seqlen,
                                                              int QKV_head) {
  ...
  // TODO: Mapping V gmem -> tid -> smem, V[kMmaAtomK,kMmaAtomN]=[16,64/128],
  // 128 threads. Mapping V gmem -> tid -> smem, V[kMmaAtomK,d]=[16,64/128], 128
  // threads.
  int load_smem_V_Bc =
      (tid / (kNumThreads / kMmaAtomK)); // kMmaAtomK 16, tid / 8, row 0~15
  int load_smem_V_d =
      (tid % (kNumThreads / kMmaAtomK)) *
      (kHeadDim / (kNumThreads / kMmaAtomK)); // (tid % 8) * 8, 0,8,56...
  // global Q row of current head for tile [Br,d] per block.
  int load_gmem_Q_Br = Q_tile_id * Br + load_smem_Q_Br;
  if (load_gmem_Q_Br >= QKV_seqlen)
    return;
  constexpr bool kIsVCanLoadIn128b =
      (kHeadDim / (kNumThreads / kMmaAtomK)) % 8 == 0;
  constexpr bool kIsVCanLoadIn64b =
      (kHeadDim / (kNumThreads / kMmaAtomK)) % 4 == 0;
  static_assert(kIsVCanLoadIn128b || kIsVCanLoadIn64b,
                "V can't load in 128b or 64b."); // 32,64,128,192,256,...
  ...
  // stage 1, Q/K smem = 64*16*2/1024=2M, V smem
  // =16*d(64|128|...)*2/1024=2M/4M/.. stage 1, total smem = max(QK_smem,
  // V_smem) = 4M if d <= 64 else V_smem. stage 1, V shared QK smem, Br=Bc=64,
  // d=64:  2M+(2M) =4M,  +Pad(2M)  = 6M stage 1, V shared QK smem, Br=Bc=128,
  // d=64:  4M+4M   =8M,  +Pad(2M)  = 10M stage 2, V shared QK smem, Br=Bc=64,
  // d=64:  4M+(4M) =8M,  +Pad(2M)  = 10M stage 2, V shared QK smem, Br=Bc=128,
  // d=64:  8M+8M   =16M, +Pad(2M)  = 18M
  ...
  // NOTE: For R_V[kWarpTileHeadDimV][2], kWarpTileHeadDimV will increase with
  // d. so, for large d, R_V will need more registers and cause performance
  // down. We have to find a way to apply MMA level tiling for V(R_V) for large
  // d. registers for current tile_K_seqlen within, [64,64] = S_tile[Br,Bc] =
        ...
        uint32_t load_smem_Q_ptr =
            (smem_Q_base_ptr +
             (stage * Q_tile_size + load_smem_Q_Br * (kMmaAtomK + kPadQ) +
              swizzle_permuted_Q_j<kMmaAtomK>(load_smem_Q_Br, load_smem_Q_d)) *
                 sizeof(half));
#pragma unroll
        ...
        uint32_t load_smem_K_ptr =
            (smem_K_base_ptr +
             (stage * K_tile_size + load_smem_K_Bc * (kMmaAtomK + kPadK) +
              swizzle_permuted_K_j<kMmaAtomK>(load_smem_K_Bc, load_smem_K_d)) *
                 sizeof(half));
#pragma unroll
...
#pragma unroll
        for (int j = 0; j < kWarpTileSeqLenK; ++j) {
          HMMA16816(R_S[0][j][0], R_S[0][j][1], R_Q[0][0], R_Q[0][1], R_Q[0][2],
                    R_Q[0][3], R_K[j][0], R_K[j][1], R_S[0][j][0],
                    R_S[0][j][1]);
        }
      }
...
#pragma unroll
      for (int stage = 0; stage < (kStage - 1); ++stage) {
        // V g2s
        int load_gmem_V_Bc = ((tile_K_seqlen * Bc) + (stage * kMmaAtomK) +
                              load_smem_V_Bc); // 0~15
        ...
        uint32_t load_smem_V_ptr =
            (smem_V_base_ptr +
             (stage * V_tile_size + load_smem_V_Bc * (kHeadDim + kPadV) +
              load_smem_V_d) *
                 sizeof(half));
    ...
    for (int tile_V_Bc = 0; tile_V_Bc < (Bc / kMmaAtomK); ++tile_V_Bc) {
      ...
      if constexpr (kStage > 1) {
        if ((tile_V_Bc + 1) < (Bc / kMmaAtomK)) {
          //  next V tile g2s
          int load_gmem_V_Bc =
              ((tile_K_seqlen * Bc) + (tile_V_Bc + 1) * kMmaAtomK +
               load_smem_V_Bc); // 0~15
          int load_gmem_V_d = load_smem_V_d;
          int load_gmem_V_addr =
              (V_gmem_offset + load_gmem_V_Bc * kHeadDim + load_gmem_V_d);
...
// 64,128,192,256,...
#pragma unroll
            for (int i = 0; i < (kHeadDim / (kNumThreads / kMmaAtomK));
                 i += 8) {
...
#pragma unroll
        ...
        int load_gmem_V_Bc = ((tile_K_seqlen * Bc) + (tile_V_Bc * kMmaAtomK) +
                              load_smem_V_Bc); // 0~15
        int load_gmem_V_d = load_smem_V_d;
        int load_gmem_V_addr =
            (V_gmem_offset + load_gmem_V_Bc * kHeadDim + load_gmem_V_d);
        uint32_t load_smem_V_ptr =
            (smem_V_base_ptr +
             (smem_sel * V_tile_size + load_smem_V_Bc * (kHeadDim + kPadV) +
              load_smem_V_d) *
                 sizeof(half));
        // headdim must be multiple of 32, (kHeadDim/8)%8==0 for 128 bits ld.
        if constexpr (kIsVCanLoadIn128b) {
...
#pragma unroll
      for (int j = 0; j < kWarpTileHeadDimV; ++j) {
        ...
        uint32_t lane_smem_V_ptr =
            (smem_V_base_ptr +
             (smem_sel * V_tile_size + lane_smem_V_Bc * (kHeadDim + kPadV) +
              lane_smem_V_d) *
                 sizeof(half));
...
void launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk(
  ...
  constexpr int kPadV = 8; // 0.25~0.5M
  ...
  cudaFuncSetAttribute(
      flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk_kernel<
...
void flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk(torch::Tensor Q,
  ...
  if (stages > 1) {
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<32, 2>(Q, K, V,
                                                                       O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<64, 2>(Q, K, V,
                                                                       O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<96, 2>(Q, K, V,
                                                                       O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<128, 2>(Q, K, V,
                                                                        O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<256, 2>(Q, K, V,
                                                                        O);
      break;
    case 512:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<512, 2>(Q, K, V,
                                                                        O);
      break;
    case 1024:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<1024, 2>(Q, K,
                                                                         V, O);
      break;
    default:
    ...
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<32, 1>(Q, K, V,
                                                                       O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<64, 1>(Q, K, V,
                                                                       O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<96, 1>(Q, K, V,
                                                                       O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<128, 1>(Q, K, V,
                                                                        O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<256, 1>(Q, K, V,
                                                                        O);
      break;
    case 512:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<512, 1>(Q, K, V,
                                                                        O);
      break;
    case 1024:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qk<1024, 1>(Q, K,
                                                                         V, O);
      break;
    default:
      throw std::runtime_error("headdim not support!");
