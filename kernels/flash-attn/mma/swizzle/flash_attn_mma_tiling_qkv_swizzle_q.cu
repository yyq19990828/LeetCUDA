...
__global__ void __launch_bounds__(WARP_SIZE *kMmaTileSeqLenQ *kMmaTileSeqLenK)
    flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q_kernel(half *Q, half *K,
                                                              half *V, half *O,
                                                              int QKV_seqlen,
                                                              int QKV_head) {
  ...
  uint32_t R_O[2]; // registers for O=PV[Br,d]=P@V, [4], only use 4 32bits
                   // registers.
...
#pragma unroll
        for (int tile_V_Bc = 0; tile_V_Bc < (Bc / kMmaAtomK); ++tile_V_Bc) {
          ...
          HMMA16816(R_O[0], R_O[1], R_S[0][w][0], R_S[0][w][1],
                    R_S[0][w + 1][0], R_S[0][w + 1][1], R_V[0], R_V[1], R_O[0],
                    R_O[1]);
        } // end for V Bc.
        if constexpr (kStage < 2) {
...
void launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q(
  ...
  cudaFuncSetAttribute(
      flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q_kernel<
...
void flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q(torch::Tensor Q,
                                                        torch::Tensor K,
                                                        torch::Tensor V,
                                                        torch::Tensor O,
                                                        int stages) {
  ...
  if (stages > 1) {
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<32, 2>(Q, K, V,
                                                                       O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<64, 2>(Q, K, V,
                                                                       O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<96, 2>(Q, K, V,
                                                                       O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<128, 2>(Q, K, V,
                                                                        O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<256, 2>(Q, K, V,
                                                                        O);
      break;
    case 512:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<512, 2>(Q, K, V,
                                                                        O);
      break;
    case 1024:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<1024, 2>(Q, K,
                                                                         V, O);
      break;
    default:
    ...
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<32, 1>(Q, K, V,
                                                                       O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<64, 1>(Q, K, V,
                                                                       O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<96, 1>(Q, K, V,
                                                                       O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<128, 1>(Q, K, V,
                                                                        O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<256, 1>(Q, K, V,
                                                                        O);
      break;
    case 512:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<512, 1>(Q, K, V,
                                                                        O);
      break;
    case 1024:
      launch_flash_attn_mma_stages_split_q_tiling_qkv_swizzle_q<1024, 1>(Q, K,
                                                                         V, O);
      break;
    default:
      throw std::runtime_error("headdim not support!");
