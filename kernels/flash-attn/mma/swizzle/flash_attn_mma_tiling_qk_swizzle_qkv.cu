...
__global__ void __launch_bounds__(WARP_SIZE *kMmaTileSeqLenQ *kMmaTileSeqLenK)
    flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv_kernel(half *Q, half *K,
                                                               half *V, half *O,
                                                               int QKV_seqlen,
                                                               int QKV_head) {
  ...
  // Mapping V gmem -> tid -> smem, V[d,kMmaAtomK]=[64/128,16], 128 threads.
  static_assert(
      kNumThreads / kHeadDim == 2,
      "tiling-qk + fully-swizzle only support kNumThreads / kHeadDim = 2!");
  int load_smem_V_d =
      (tid / (kNumThreads / kHeadDim)); // d 64, tid / 2, row 0~64, make sure
                                        // kNumThreads / kHeadDim = 2
  int load_smem_V_Bc =
      (tid % (kNumThreads / kHeadDim)) *
      (kMmaAtomK / (kNumThreads / kHeadDim)); // (tid % 2) * 8, 0,8
  ...
  constexpr int V_tile_size =
      kHeadDim *
      (kMmaAtomK + kPadV); // V[d,16], V[d,Bc], means V[Bc,d] in col major
...
#pragma unroll
        for (int i = 0; i < (kMmaAtomK / (kNumThreads / kHeadDim)); i += 8) {
          uint32_t load_smem_V_ptr =
              (smem_V_base_ptr +
               (stage * V_tile_size + load_smem_V_d * (kMmaAtomK + kPadV) +
...
#pragma unroll
        for (int i = 0; i < (kMmaAtomK / (kNumThreads / kHeadDim)); i += 8) {
          uint32_t load_smem_V_ptr =
              (smem_V_base_ptr +
               (smem_sel * V_tile_size + load_smem_V_d * (kMmaAtomK + kPadV) +
                swizzle_permuted_V_j<kMmaAtomK>(
                    load_smem_V_d, (load_smem_V_Bc + i) % kMmaAtomK)) *
                   sizeof(half));
          CP_ASYNC_CG(load_smem_V_ptr, &V[load_gmem_V_addr + i], 16);
...
#pragma unroll
      for (int j = 0; j < kWarpTileHeadDimV; ++j) {
        ...
        uint32_t lane_smem_V_ptr =
            (smem_V_base_ptr +
             (smem_sel * V_tile_size + lane_smem_V_d * (kMmaAtomK + kPadV) +
...
void launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv(
    torch::Tensor Q, torch::Tensor K, torch::Tensor V, torch::Tensor O) {
  // Now: fixed tile BrxBc=128x128 for d>= 128, 64x64 for d<128.
  constexpr int kMmaAtomM = 16;
  constexpr int kMmaAtomN = 8;
  constexpr int kMmaAtomK = 16;
  // (WARP_SIZE * kMmaTileSeqLenQ)/kHeadDim=2,
  // kMmaTileSeqLenQ=(2*kHeadDim)/WARP_SIZE d=64, 4; d=128, 8; d=32, 2; d=256,
  // 16;
  constexpr int kMmaTileSeqLenQ = (2 * kHeadDim) / WARP_SIZE;
  ...
  constexpr int V_smem_size = (kStage * (kHeadDim * (kMmaAtomK + kPadV)));
  ...
  cudaFuncSetAttribute(
      flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv_kernel<
...
void flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv(torch::Tensor Q,
  ...
  if (stages > 1) {
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv<32, 2>(Q, K, V,
                                                                        O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv<64, 2>(Q, K, V,
                                                                        O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv<96, 2>(Q, K, V,
                                                                        O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv<128, 2>(Q, K,
                                                                         V, O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv<256, 2>(Q, K,
                                                                         V, O);
      break;
    default:
    ...
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv<32, 1>(Q, K, V,
                                                                        O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv<64, 1>(Q, K, V,
                                                                        O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv<96, 1>(Q, K, V,
                                                                        O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv<128, 1>(Q, K,
                                                                         V, O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_tiling_qk_swizzle_qkv<256, 1>(Q, K,
                                                                         V, O);
      break;
    default:
      throw std::runtime_error("headdim not support!");
