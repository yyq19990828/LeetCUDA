...
__global__ void __launch_bounds__(WARP_SIZE *kMmaTileSeqLenQ *kMmaTileSeqLenK)
    flash_attn_mma_stages_split_q_shared_qkv_Os2g_kernel(half *Q, half *K,
                                                         half *V, half *O,
                                                         int QKV_seqlen,
                                                         int QKV_head) {
  ...
  constexpr bool kCanPrefetchQs2r =
      ((kHeadDim / kMmaAtomK) <= 16); // always true.
  ...
  constexpr bool kCanPrefetchKVg2s =
      (((Q_tile_size /
         (K_tile_size > V_tile_size ? K_tile_size : V_tile_size)) >= 2) &&
       (kStage >= 2));
...
// By the way, we have to reduce R_Z to 0 regs and reuse R_Q for collective
// store. Then we can load Q from smem only once and reuse it for <loop over K
// seqlen> processes. This will reduce large io-access for Q smem while N is
// large.
#pragma unroll
          for (int i = 0; i < kWarpTileSeqLenQ; ++i) { // Q[Br,d]=[M,K]
            ...
            int lane_smem_Q_d =
                tile_K_d * kMmaAtomK + (lane_id / 16) * 8; // 0,8
...
#pragma unroll
        for (int j = 0; j < kWarpTileSeqLenK; ++j) {
          HMMA16816(R_S[i][j][0], R_S[i][j][1], R_Q[tile_K_d][i][0],
                    R_Q[tile_K_d][i][1], R_Q[tile_K_d][i][2],
                    R_Q[tile_K_d][i][3], R_K[j][0], R_K[j][1], R_S[i][j][0],
                    R_S[i][j][1]);
        }
      }
...
// Store O(D): Write O[Br,d] from regs -> smem, smem -> gmem
// reuse Q smem [Br,d] as O smem [Br,d], e.g [128,64]
// 0. O r2s: regs -> smem
#pragma unroll
  for (int i = 0; i < kWarpTileSeqLenP; ++i) { // 1
#pragma unroll
    for (int j = 0; j < kWarpTileHeadDimV; ++j) { // 8
      // store m16n8 matrix r2s.
      ...
      int store_lane_smem_O_Br = store_warp_regs_O_Br + (lane_id / 4); // 0~7
      int store_lane_smem_O_d =
          store_warp_regs_O_d + (lane_id % 4) * 2; // 0,2,4,6
      int store_smem_O_addr_0 =
          (store_lane_smem_O_Br + 0) * (kHeadDim + kPadQ) + store_lane_smem_O_d;
      int store_smem_O_addr_1 =
          (store_lane_smem_O_Br + 8) * (kHeadDim + kPadQ) + store_lane_smem_O_d;
      LDST32BITS(Q_tile_smem[store_smem_O_addr_0]) = LDST32BITS(R_D[i][j][0]);
      LDST32BITS(Q_tile_smem[store_smem_O_addr_1]) = LDST32BITS(R_D[i][j][1]);
      // uint32_t R_Z[2][4];
      ...
      // if (lane_id % 4 == 0) {
      //   int store_warp_regs_O_Br = warp_QP * (kMmaAtomM * kWarpTileSeqLenP )
      //   + i * kMmaAtomM; int store_warp_regs_O_d  = warp_KV * (kMmaAtomN *
      //   kWarpTileHeadDimV) + j * kMmaAtomN; int store_lane_smem_O_Br =
      //   store_warp_regs_O_Br + (lane_id / 4); // 0~7 int store_lane_smem_O_d
      //   = store_warp_regs_O_d; int store_smem_O_addr_0  =
      //   (store_lane_smem_O_Br + 0) * (kHeadDim + kPadQ) +
      //   store_lane_smem_O_d; int store_smem_O_addr_1  = (store_lane_smem_O_Br
      //   + 8) * (kHeadDim + kPadQ) + store_lane_smem_O_d;
      //   LDST128BITS(Q_tile_smem[store_smem_O_addr_0]) =
      //   LDST128BITS(R_Z[0][0]); LDST128BITS(Q_tile_smem[store_smem_O_addr_1])
      //   = LDST128BITS(R_Z[1][0]);
      // }
    } // end for kWarpTileHeadDimV
  } // end for kWarpTileSeqLenQ
  ...
  // 1. O s2g: smem -> gmem, st.128 bits, [Br,d], e.g [128,64]
  int store_smem_O_addr = (load_smem_Q_Br * (kHeadDim + kPadQ) + load_smem_Q_d);
  int store_gmem_O_addr =
      ((O_gmem_offset) + (O_tile_id * Br + load_smem_Q_Br) * kHeadDim +
       (load_smem_Q_d));
#pragma unroll
  for (int i = 0; i < (kHeadDim / (kNumThreads / Br)); i += 8) {
    LDST128BITS(O[store_gmem_O_addr + i]) =
        LDST128BITS(Q_tile_smem[store_smem_O_addr + i]);
  }
}

template <const int kHeadDim, const int kStage>
void launch_flash_attn_mma_stages_split_q_shared_qkv_Os2g(torch::Tensor Q,
                                                          torch::Tensor K,
                                                          torch::Tensor V,
                                                          torch::Tensor O) {
  ...
  // block, QKV smem size, QKV fully shared the same smem.
  ...
  flash_attn_mma_stages_split_q_shared_qkv_Os2g_kernel<
      ...
      kMmaTileSeqLenK, kMmaTileSeqLenP, kMmaTileHeadDimV, kWarpTileSeqLenQ,
      kWarpTileSeqLenK, kWarpTileSeqLenP, kWarpTileHeadDimV, kStage, kPadQ,
      kPadK, kPadV><<<grid, block, smem_max_size>>>(
      reinterpret_cast<half *>(Q.data_ptr()),
      reinterpret_cast<half *>(K.data_ptr()),
      reinterpret_cast<half *>(V.data_ptr()),
      reinterpret_cast<half *>(O.data_ptr()), QKV_seqlen, QKV_head);
}
...
void flash_attn_mma_stages_split_q_shared_qkv_Os2g(torch::Tensor Q,
  ...
  if (stages > 1) {
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_shared_qkv_Os2g<32, 2>(Q, K, V, O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_shared_qkv_Os2g<64, 2>(Q, K, V, O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_shared_qkv_Os2g<96, 2>(Q, K, V, O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_shared_qkv_Os2g<128, 2>(Q, K, V, O);
      break;
    default:
    ...
    switch (d) {
    case 32:
      launch_flash_attn_mma_stages_split_q_shared_qkv_Os2g<32, 1>(Q, K, V, O);
      break;
    case 64:
      launch_flash_attn_mma_stages_split_q_shared_qkv_Os2g<64, 1>(Q, K, V, O);
      break;
    case 96:
      launch_flash_attn_mma_stages_split_q_shared_qkv_Os2g<96, 1>(Q, K, V, O);
      break;
    case 128:
      launch_flash_attn_mma_stages_split_q_shared_qkv_Os2g<128, 1>(Q, K, V, O);
      break;
    case 256:
      launch_flash_attn_mma_stages_split_q_shared_qkv_Os2g<256, 1>(Q, K, V, O);
      break;
    default:
      throw std::runtime_error("headdim not support!");
      break;
